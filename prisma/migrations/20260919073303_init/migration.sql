...
-- CreateIndex
CREATE INDEX "Collection_userId_updatedAt_idx" ON "Collection"("userId", "updatedAt" DESC);
...
-- "ItemType_userId_slug_key" above does not constrain system types: Postgres
-- treats every NULL as distinct, so any number of rows could share a slug while
-- "userId" IS NULL. This partial index enforces one system type per slug.
